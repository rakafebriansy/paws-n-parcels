...
class DeliverySystem {
    ...
    func pickUpPackage(request: Request, for entity: GKEntity) {
        guard let component = entity.component(ofType: DeliveryComponent.self)
            ...
            debugLog("[DeliverySystem] Error: Entity does not have a DeliveryComponent. Pickup aborted.")
            return
        }
        
        guard !component.isHoldingPackage
        else {
            debugLog("[DeliverySystem] Warning: Entity is already holding a package. Cannot pick up another one.")
            return
        }
        
        component.activeRequest = request
        self.activePackage = request
        debugLog("[DeliverySystem] Package picked up successfully. Sender: \(request.sender.name), Receiver: \(request.receiver.name).")
    }
    ...
    func deliverPackage(for entity: GKEntity, relationships: [AnimalRelationship]) -> (pointsAdded: Int, isLevelUp: Bool, unlockedItem: Collectible?) {
        guard let component = entity.component(ofType: DeliveryComponent.self),
            ...
            debugLog("[DeliverySystem] Error: Entity is not holding any active request to deliver.")
            return (0, false, nil)
        }
    ...
    private func processDeliveryReward(for request: Request, in relationships: [AnimalRelationship]) -> (pointsAdded: Int, isLevelUp: Bool, unlockedItem: Collectible?) {
        guard let relationship = relationships.first(where: {
            ...
            debugLog("[DeliverySystem] Warning: No relationship found between \(request.sender.name) and \(request.receiver.name). Package delivered but no points awarded.")
        ...
        if isLevelUp {
            debugLog("[DeliverySystem] Level Up! Friendship between \(relationship.friendOne.name) and \(relationship.friendTwo.name) reached level \(newLevel.intValue).")
            if let context = modelContext {
                ...
                if let lockedItems = try? context.fetch(descriptor),
                   let nextItem = lockedItems.first {
                    ...
                    debugLog("[DeliverySystem] New Collectible Unlocked: \(nextItem.name)")
                }
            } else {
                debugLog("[DeliverySystem] Warning: modelContext is nil. Cannot unlock collectible.")
            }
        }
        
        GameDataManager.shared.save()
        debugLog("[DeliverySystem] Package delivered. \(GameConfig.deliveryRewardPoints) points added. Database saved.")
